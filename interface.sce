// ==============================================
// Projet : Interface Image - ANTON NELCON Steve
// =============================================
...
f = figure("Position", [100, 100, 1200, 600], ...
           "Figure_name", "Outils libre - ANTON NELCON Steve Master IBD", ...
           "BackgroundColor", [0.9 0.9 0.95]);
        

// Slider (position inchangée)
scroll_bar = uicontrol(f, ...
    "style", "slider", ...
    "position", [10, 170, 50, 445], ...
    "min", 0, ...
    "max", 100, ...
    "value", 50, ...
    "callback", "scroll_callback()");

// Stocker le handle du slider dans la figure
f.user_data = scroll_bar;

// Flèche en haut (au bord supérieur)
btn_up = uicontrol(f, ...
    "style", "pushbutton", ...
    "string", "▲", ...
    "position", [10, 620, 50, 30], ...
    "callback", "scroll_up()");

// Flèche en bas (au bord inférieur)
btn_down = uicontrol(f, ...
    "style", "pushbutton", ...
    "string", "▼", ...
    "position", [10, 140, 50, 30], ...
    "callback", "scroll_down()");

// Callback du slider
function scroll_callback()
    sb = gcf().user_data;              // récupérer le slider
    val = sb.value;
    disp("Valeur : " + string(val));
endfunction

// Flèche haut
function scroll_up()
    f = gcf();
    sb = f.user_data;                  // slider depuis la figure
    sb.value = min(sb.value + 1, sb.max);
    sb.value = sb.value;               // forcer le redraw
    scroll_callback();
endfunction
...
function scroll_down()
    f = gcf();
    sb = f.user_data;
    sb.value = max(sb.value - 1, sb.min);
    sb.value = sb.value;               // forcer le redraw
    scroll_callback();
endfunction
...
titre_projet = uicontrol(f, "Style", "text", ...
          "String", "Projet réalisé par Steve ANTON NELCON - M1 IBD", ...
...
function activer_mode_clair()
    set(f, "BackgroundColor", [0.9 0.9 0.95]);
...
bouton_langue = uicontrol(f, ...
    "Style", "pushbutton", ...
    "String", "FR / EN", ...
    "Position", [800, 150, 130, 40], ...
    "FontSize", 12, ...
    "BackgroundColor", [0.9 0.4 0.8], ...
    "ForegroundColor", [0 0 0], ...
...
function activer_anglais()
    set(titre_projet, "String", "Project by Steve ANTON NELCON - M1 Big Data");
...
function activer_francais()
    set(titre_projet, "String", "Projet réalisé par Steve ANTON NELCON - M1 IBD");
...
// "Steve ANTON NELCON" en noir normal
uicontrol(f2, ...
    "Style", "text", ...
    "String", "Steve ANTON NELCON", ...
...
// "23 novembre 2025" en noir normal
uicontrol(f2, ...
    "Style", "text", ...
    "String", "23 novembre 2025", ...
    "Position", [150, 450, 200, 20], ...
    "FontSize", 12, ...
    "FontWeight", "normal", ...
    "ForegroundColor", [0 0 0], ...       // Noir
    "BackgroundColor", [0.93 0.93 0.95], ...
    ...
    uicontrol(f2, ...
     ...
     uicontrol(f2, ...
        "Style", "text", ...
        "String", "  - Barre verticale (inutile) ''▲'' : élément décoratif", ...
        "Position", [70, 220, 1000, 20], ...
        "FontSize", 12, ...
        "ForegroundColor", [0.1 0.1 0.1], ...
        "BackgroundColor", [0.93 0.93 0.95], ...
        "HorizontalAlignment", "left");
